...
Мы хотим, чтобы любой контрибьютор (прямой автор или агент‑раннер) мог:

1) выбрать задачу из план‑файла в репозитории;  
2) подготовить PR, который **объективно** проходит набор гейтов качества;  
3) получить понятный фидбэк (CI/ревью) и довести PR до состояния “не стыдно показать ревьюверу”.
...
- **Contributor** — любой автор PR и/или его агент‑раннер.
...
**Правило:** изменения публичного API / форматов / совместимости требуют ревью мейнтейнера.
...
Рекомендуемый принцип: **агент предлагает, владелец (PO/maintainer) утверждает**.
...
- `needs-maintainer` — требуется решение мейнтейнера/PO
- `blocked` — есть блокер
